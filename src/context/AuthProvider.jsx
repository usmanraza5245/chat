...
const AuthProvider = ({ children }) => {
  ...
  const setAuthenticatedUserFromCredentialExchangeService = (response) => {
    ...
    AWS.config.credentials.needsRefresh = function () {
      return (
        Date.now() - credentialReceiveTime > CREDENTIAL_ROLE_EXPIRY_DURATION
      );
    };

    AWS.config.credentials.refresh = function (cb) {
      console.error(
        "Credentials expired via anonymous token used for Demo. TODO: implement refresh for actual production use case"
      );
